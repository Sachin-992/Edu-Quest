-- ============================================================
-- EDUQUEST — MIGRATION PART 2 of 2
-- New Project: oeaowgbycenftvhwonyb
--
-- ⚠️  Run MIGRATION_PART1_ENUM.sql FIRST, then run THIS file.
--
-- This file creates all tables, functions, triggers,
-- RLS policies, indexes, seed data, and admin setup.
-- ============================================================
...
-- FK from schools → subscription_plans
DO $$ BEGIN
...
DO $$ BEGIN
  ALTER TABLE public.student_progress ADD CONSTRAINT fk_student_progress_user
    FOREIGN KEY (user_id) REFERENCES auth.users(id) ON DELETE CASCADE;
EXCEPTION WHEN duplicate_object THEN NULL; END $$;

DO $$ BEGIN
  ALTER TABLE public.adventure_progress ADD CONSTRAINT fk_adventure_progress_user
    FOREIGN KEY (user_id) REFERENCES auth.users(id) ON DELETE CASCADE;
EXCEPTION WHEN duplicate_object THEN NULL; END $$;

DO $$ BEGIN
  ALTER TABLE public.coin_transactions ADD CONSTRAINT fk_coin_transactions_user
    FOREIGN KEY (user_id) REFERENCES auth.users(id) ON DELETE CASCADE;
EXCEPTION WHEN duplicate_object THEN NULL; END $$;

DO $$ BEGIN
  ALTER TABLE public.study_sessions ADD CONSTRAINT fk_study_sessions_user
    FOREIGN KEY (user_id) REFERENCES auth.users(id) ON DELETE CASCADE;
EXCEPTION WHEN duplicate_object THEN NULL; END $$;

DO $$ BEGIN
  ALTER TABLE public.student_avatar_items ADD CONSTRAINT fk_student_avatar_items_user
    FOREIGN KEY (user_id) REFERENCES auth.users(id) ON DELETE CASCADE;
EXCEPTION WHEN duplicate_object THEN NULL; END $$;
...
-- (All new enum values are now safely committed from Part 1)
-- ══════════════════════════════════════════════════════════════
...
-- Uses text cast to safely compare with 'platform_admin'
...
CREATE TRIGGER update_adventure_progress_updated_at BEFORE UPDATE ON public.adventure_progress
  FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();

DROP TRIGGER IF EXISTS update_leaderboard_settings_updated_at ON public.leaderboard_settings;
CREATE TRIGGER update_leaderboard_settings_updated_at BEFORE UPDATE ON public.leaderboard_settings
  FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
...
CREATE POLICY "Admins can manage roles"
  ...
  USING (has_role(auth.uid(), 'super_admin') OR is_platform_admin(auth.uid()));
...
-- ══════════════════════════════════════════════════════════════
-- SECTION 9: ADMIN USER SETUP
--
-- ⚠️  BEFORE RUNNING THIS SECTION:
--   1. Go to: Supabase Dashboard → Authentication → Users → Add User
--   2. Enter your NEW admin email and password
--   3. Check "Auto Confirm User" ✅
--   4. Click Create User
--   5. Then update the email below and run this section
--
-- ⬇️  CHANGE THIS EMAIL TO YOUR NEW ADMIN EMAIL ⬇️
-- ══════════════════════════════════════════════════════════════

DO $$
DECLARE
  v_admin_email TEXT := 'sachinchinnasamy2021@gmail.com';
  admin_uid UUID;
  demo_school_id UUID;
BEGIN
  SELECT id INTO admin_uid FROM auth.users WHERE email = v_admin_email;
  SELECT id INTO demo_school_id FROM public.schools WHERE code = 'DEMO001' LIMIT 1;

  IF admin_uid IS NULL THEN
    RAISE NOTICE '⚠️  Admin user "%" not found. Create the user in Auth dashboard first, then re-run this section.', v_admin_email;
  ELSE
    ...
    RAISE NOTICE '✅ Admin setup complete! email=%, user_id=%, role=super_admin', v_admin_email, admin_uid;
  END IF;
END $$;
...
SELECT '✅ PART 2 MIGRATION COMPLETE' AS status;
