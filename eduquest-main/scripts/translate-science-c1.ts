...
const translations: Record<string, { q_ta: string; opts_ta: string[] }> = {
    // Easy
...
async function translateScienceClass1() {
    const { data: quiz, error: qErr } = await supabase
        .from("quizzes")
        .select("id")
        .eq("title", "Science Practice Quiz — Class 1")
        .single();
    ...
    if (qQsErr || !questions) return;

    console.log(`Translating all ${questions.length} Science questions...`);

    for (const q of questions) {
        const trans = translations[q.question_text];
        if (trans) {
            await supabase
                .from("quiz_questions")
                .update({
                    question_text_tamil: trans.q_ta,
                    options_tamil: trans.opts_ta
                })
                .eq("id", q.id);
            console.log("✅ Deep Translated:", q.question_text);
        }
    }

    console.log("Translation complete! The UI will now look exactly like the screenshot without fake tags.");
}

translateScienceClass1();
